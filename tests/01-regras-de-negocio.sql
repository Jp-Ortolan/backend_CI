...
-- ---------------------------------------------------------------------------
-- Trigger de novo usuário (migration 008). Só roda onde auth.users existe:
-- Supabase de verdade ou local/CI com o stub aplicado.
do $$
declare novo_id uuid; papel_criado papel_usuario; nome_criado text;
begin
  if to_regclass('auth.users') is null then
    raise notice 'PULADO — auth.users não existe neste ambiente';
    return;
  end if;

  insert into auth.users (email, raw_user_meta_data)
  values ('teste.trigger@centroinovacao.br', '{"nome":"Fulano de Teste"}'::jsonb)
  returning id into novo_id;

  select papel, nome into papel_criado, nome_criado from usuario where id = novo_id;

  if papel_criado is null then
    raise exception 'FALHOU: cadastro no Auth não criou a linha em usuario';
  end if;
  if papel_criado <> 'leitura' then
    raise exception 'FALHOU: novo usuário deveria nascer com papel leitura, veio %', papel_criado;
  end if;
  if nome_criado <> 'Fulano de Teste' then
    raise exception 'FALHOU: nome do metadata não foi aproveitado, veio %', nome_criado;
  end if;
  raise notice 'OK   auth — novo cadastro virou usuario com papel leitura';

  delete from usuario where id = novo_id;
